...
const BearbeitenContent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [guideRes, categoriesRes] = await Promise.all([
        ...
        if (guideData.success) {
          ...
          setFormData({
            ...
            slug: guideData.guide.slug,
            excerpt: guideData.guide.excerpt,
            content: guideData.guide.content,
            categoryId: guideData.guide.categoryId,
            coverImage: guideData.guide.coverImage || '',
            published: guideData.guide.published,
          })
        }
    ...
    return (
      <div className='flex min-h-dvh items-center justify-center'>
        <div className='text-center'>
          <h1 className='text-2xl font-bold mb-2'>Anleitung nicht gefunden</h1>
          <Button asChild>
            <a href='/dashboard/anleitungen'>Zurück zur Übersicht</a>
          </Button>
        </div>
      </div>
    )
              ...
              <div className='flex items-center gap-4'>
                ...
                <div>
                  <div className='flex items-center gap-2'>
                    <h1 className='text-2xl font-bold'>Anleitung bearbeiten</h1>
                    <Badge variant={formData.published ? 'default' : 'secondary'}>
                      {formData.published ? 'Veröffentlicht' : 'Entwurf'}
                    </Badge>
                  </div>
                  <p className='text-muted-foreground'>Erstellt am {new Date(guide.createdAt).toLocaleDateString('de-DE')}</p>
                </div>
              </div>
              <div className='flex items-center gap-2'>
                {formData.published ? (
                  ...
                  <Button variant='outline' onClick={() => handleSave()} disabled={saving || !isValid}>
                    {saving ? (
                      <Loader2Icon className='size-4 mr-2 animate-spin' />
                    ) : (
                ...
                <Button onClick={() => handleSave(true)} disabled={saving || !isValid}>
                  ...
                  {formData.published ? 'Aktualisieren' : 'Veröffentlichen'}
                </Button>
              </div>
            </div>
